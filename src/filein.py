...
class filein:
    ...
    def __init__(self, filename, shape, recl, rec, dtype, kind, endian, recstep):
        ...
        self.__kind = self.get_kind(dtype, kind)
        #self.__kind = kind
    ...
    def get_kind(self, dtype, kind):
        ...
        if (dtype == 'int'):
            ...
            raise ValueError('dtype argument must be real or int, but input is {}'.format(dtype))

        return '{}{}'.format(label, kind)
        ...
        #    return 'l'
        #else:
        #    raise ValueError('kind argument must be real32, real64, int32, or int64, but input is {}'.format(kind))
    ...
    def __argcheck(self, filename, shape, recl, rec, kind, recstep):
        ...
        #if (np.prod(shape)*4 != recl):
        if (np.prod(shape)*4 != recl and np.prod(shape)*8 != recl):
            print('')
